...
async function showOnlineMenu() {
    ...
    document.getElementById('main-cards').style.display = 'none'; document.getElementById('original-buttons').style.display = 'none';
    ui.showPanel('online-menu'); updateRejoinButtonsFromDB();
}
...
function startWaitingPoll(gameId) {
    ...
    waitingPollInterval = setInterval(async () => {
        try {
            ...
            if (!data) return;
            if (data.status === 'countdown') { stopWaitingPoll(); currentOnlineGame = data; ui.showCountdown(data.host_nickname, data.room_code); }
            else if (data.status === 'active') { stopWaitingPoll(); currentOnlineGame = data; startOnlineGame(); }
            else if (data.status === 'cancelled' || data.status === 'terminated') { stopWaitingPoll(); ui.toast('Game was cancelled.'); resetOnlineState(); ui.showMenu(); }
        } catch (e) {}
    }, 1000);
}
...
async function startOnlineGame() {
    ...
    ui.hideAllPanels(); ui.showGameUI(); ui.setChatVisibility(true); ui.resetChatMessageCount(); ui.setOnlineBottomButtons(true);
    engine.setMyColor(myColor); engine.startGame('online'); started = true; gameMode = 'online'; over = false; frozen = false; engine.setFrozen(false);
    lastTimerSync = Date.now(); startOnlineGameLoop(); startChatPolling(currentOnlineGame.id); engine.rotateForPlayer(myColor);
}
...
async function pollGameState() {
    ...
    try {
        ...
        if (gameData.status === 'terminated') { if (currentUserId !== gameData.host_player_id) ui.toast('Match terminated by the host.'); resetOnlineState(); ui.showMenu(); return; }
        if (gameData.status === 'frozen') { if (!frozen) { frozen = true; engine.setFrozen(true); if (currentUserId !== gameData.leaver_id) { ui.toast('Opponent left – waiting for rejoin…'); sessionStorage.setItem('chess3d_frozen_game', gameData.id); } else sessionStorage.setItem('chess3d_frozen_game', gameData.id); } return; }
        ...
        if (serverState !== lastKnownServerState) { lastKnownServerState = serverState; engine.syncBoardFromServer(gameData.board_state.brd, gameData.board_state.turn, gameData.board_state.cas, gameData.board_state.ep, gameData.timer_w, gameData.timer_b); }
    } catch (e) {}
}
...
async function exitOnlineGame() {
    if (!currentOnlineGame) return;
    stopOnlineGameLoop(); stopChatPolling(); const gameId = currentOnlineGame.id;
    if (currentOnlineGame.host_player_id === currentUserId) { await db.terminateGame(gameId); resetOnlineState(); ui.showMenu(); }
    else { await db.freezeGame(gameId, currentUserId); sessionStorage.setItem('chess3d_frozen_game', gameId); resetOnlineState(); ui.showMenu(); updateRejoinButtonsFromSession(); }
}
function resetOnlineState() {
    ...
    engine.setFrozen(false); ui.hideGameUI(); ui.hideGameOver(); engine.resetState(); started = false; gameMode = null;
}
