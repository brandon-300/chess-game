// main.js — Orchestrator for Chess 3D (v30 – game-over popup fixes)
                ...
                if (info) {
                    ...
                    if (gameMode === 'ai') {
                        ...
                        if (info.resultType === 'draw') {
                            ...
                            const winner = info.resultType === 'Red' ? 'Red' : 'Black';
                            title = winner + ' Wins!';
                            subtitle = info.title || '';
                        }
                    } else {
                    ...
                    over = true;
                    // Auto-return for offline modes after 5s
                    if (!isOnline) {
                        setTimeout(() => {
                            ui.hideGameOver();
                            ui.showMenu();
...
async function startOnlineGame() {
    ...
    ui.hideAllPanels(); ui.showGameUI(); ui.setChatVisibility(true); ui.resetChatState(); ui.setOnlineBottomButtons(true);
...
async function pollGameState() {
    ...
    try {
        ...
        if (gameData.status === 'frozen') {
            if (!frozen) { frozen = true; engine.setFrozen(true); if (currentUserId !== gameData.leaver_id) { ui.toast('Opponent left – waiting for rejoin…'); sessionStorage.setItem('chess3d_frozen_game', gameData.id); } else sessionStorage.setItem('chess3d_frozen_game', gameData.id); }
            return;
        }
...
async function sendChat(msg) {
    ...
    if (row) { ui.appendChatMessage(nickname, msg, true); ui.registerOwnMessage(row.id); }
    else { ui.appendChatMessage(nickname, msg, true); }
}
