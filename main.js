...
async function syncOfflineToCloud() {
    ...
    if (!currentUserId) { screenLog('→ "Please log in" toast'); ui.toast('Please log in to sync data.', 3000); return; }
    if (!navigator.onLine) { screenLog('→ "No internet" toast'); ui.toast('No internet connection.', 3000); return; }
    if (!aiBackup && !pvpBackup) { screenLog('→ "No offline data" toast'); ui.toast('No offline data to sync.', 4000); return; }
...
async function deleteAllSyncedData() {
    screenLog('deleteAllSyncedData entered');
    if (!currentUserId) { screenLog('→ not logged in'); ui.toast('Please log in to delete synced data.', 3000); return; }
    if (!navigator.onLine) { screenLog('→ offline'); ui.toast('No internet connection.', 3000); return; }
    screenLog('→ calling db.deleteAllSyncedData…');
    try { await db.deleteAllSyncedData(currentUserId); screenLog('→ success'); ui.toast('Cloud data deleted.', 3000); } catch (e) { screenLog('→ failed: ' + e.message); ui.toast('Delete failed: ' + e.message, 4000); }
}
