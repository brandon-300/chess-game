...
function updateRejoinButtonsFromSession() { const frozenId = sessionStorage.getItem('chess3d_frozen_game'); ui.setRejoinButtonsVisibility(!!frozenId, !!frozenId); if (frozenId) validateRejoinButtonAsync(frozenId); }
async function validateRejoinButtonAsync(frozenId) { if (!db) return; try { const game = await db.fetchGameState(frozenId); if (!game || game.status !== 'frozen' || Date.now() - new Date(game.leave_time).getTime() > 10 * 60 * 1000) { sessionStorage.removeItem('chess3d_frozen_game'); ui.setRejoinButtonsVisibility(false, false); } } catch (e) {} }
async function updateRejoinButtonsFromDB() { if (!currentUserId || !db) { ui.setRejoinButtonsVisibility(false, false); return; } const frozenGame = await db.getFrozenGameForUser(currentUserId); const hasFrozen = !!frozenGame; ui.setRejoinButtonsVisibility(hasFrozen, hasFrozen); if (hasFrozen) sessionStorage.setItem('chess3d_frozen_game', frozenGame.id); else sessionStorage.removeItem('chess3d_frozen_game'); }
async function rejoinPublicGame() { if (!currentUserId) return; const frozenId = sessionStorage.getItem('chess3d_frozen_game'); if (!frozenId) { ui.toast('No frozen game found.'); return; } try { const game = await db.unfreezeGame(frozenId, currentUserId); enterOnlineGame(game); } catch (e) { ui.toast('Rejoin failed: ' + e.message); } }
function enterOnlineGame(game) { currentOnlineGame = game; myColor = (game.host_player_id === currentUserId) ? 'w' : 'b'; sessionPlayerKey = myColor === 'w' ? game.host_player_key : game.joiner_player_key; sessionStorage.setItem('chess3d_playerkey_' + game.id, sessionPlayerKey); sessionStorage.removeItem('chess3d_frozen_game'); ui.hideAllPanels(); ui.showGameUI(); ui.setChatVisibility(true); ui.setOnlineBottomButtons(true); ui.setRejoinButtonsVisibility(false, false); engine.setMyColor(myColor); engine.setGameMode('online'); engine.syncBoardFromServer(game.board_state.brd, game.board_state.turn, game.board_state.cas, game.board_state.ep, game.timer_w, game.timer_b); started = true; gameMode = 'online'; over = false; frozen = false; engine.setFrozen(false); lastTimerSync = Date.now(); startOnlineGameLoop(); startChatPolling(game.id); engine.rotateForPlayer(myColor); }
