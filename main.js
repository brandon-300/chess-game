...
let moveSyncing = false, lastKnownServerState = null, pollInterval = null, rematchCountdownInterval = null, waitingPollInterval = null, lastTimerSync = 0;
...
async function init() {
    ...
    try {
        ...
        ui.initUI({
            ...
            onToggleMic: () => toggleMic(), onToggleSpeaker: () => toggleSpeaker(),
...
function enterOnlineGame(game) {
    ...
    ui.hideAllPanels(); ui.showGameUI(); ui.setOnlineBottomButtons(true); ui.setRejoinButtonsVisibility(false, false);
...
// Offline flow
function startOfflineGame(mode) { gameMode = mode; ui.hideAllPanels(); ui.showGameUI(); ui.setVoiceControlsVisibility(false); ui.setOnlineBottomButtons(false); started = true; engine.startGame(mode); over = false; if (mode === 'ai' && engine.getPlayerColor() === 'b') engine.scheduleAI(500); startAutoSave(); }
...
async function startOnlineGame() { if (!currentOnlineGame) return; document.getElementById('ms').style.display = 'none'; if (currentOnlineGame.host_player_id === currentUserId) await db.updateGameStatus(currentOnlineGame.id, 'active'); ui.hideAllPanels(); ui.showGameUI(); ui.setOnlineBottomButtons(true); engine.setMyColor(myColor); engine.startGame('online'); started = true; gameMode = 'online'; over = false; frozen = false; engine.setFrozen(false); lastTimerSync = Date.now(); startOnlineGameLoop(); startVoice(); engine.rotateForPlayer(myColor); }

// Voice call
function getOpponentNickname() { if (!currentOnlineGame) return ''; return currentOnlineGame.host_player_id === currentUserId ? currentOnlineGame.joiner_nickname : currentOnlineGame.host_nickname; }
function startVoice() {
    if (!currentOnlineGame) return;
    const isPolite = currentOnlineGame.host_player_id !== currentUserId; // joiner defers on offer collisions
...
async function pollGameState() { if (!currentOnlineGame || moveSyncing || over) return; try { const gameData = await db.fetchGameState(currentOnlineGame.id); if (!gameData) return; if (gameData.status === 'terminated') { if (currentUserId !== gameData.host_player_id) ui.toast('Match terminated by the host.'); resetOnlineState(); ui.showMenu(); return; } if (gameData.status === 'frozen') { if (!frozen) { frozen = true; engine.setFrozen(true); if (voice.isMicOn()) { voice.disableMic(); ui.setMicState(false); } if (currentUserId !== gameData.leaver_id) { ui.toast('Opponent left – waiting for rejoin…'); sessionStorage.setItem('chess3d_frozen_game', gameData.id); } else sessionStorage.setItem('chess3d_frozen_game', gameData.id); } return; } if (frozen && gameData.status === 'active') { frozen = false; engine.setFrozen(false); ui.toast('Opponent rejoined!'); sessionStorage.removeItem('chess3d_frozen_game'); stopVoice(); startVoice(); } const serverState = JSON.stringify(gameData.board_state); if (serverState !== lastKnownServerState) { lastKnownServerState = serverState; engine.syncBoardFromServer(gameData.board_state.brd, gameData.board_state.turn, gameData.board_state.cas, gameData.board_state.ep, gameData.timer_w, gameData.timer_b); } } catch (e) {} }
...
async function exitOnlineGame() { if (!currentOnlineGame) return; stopOnlineGameLoop(); stopVoice(); const gameId = currentOnlineGame.id; if (currentOnlineGame.host_player_id === currentUserId) { await db.terminateGame(gameId); resetOnlineState(); ui.showMenu(); } else { await db.freezeGame(gameId, currentUserId); sessionStorage.setItem('chess3d_frozen_game', gameId); resetOnlineState(); ui.showMenu(); updateRejoinButtonsFromSession(); } }
function resetOnlineState() { stopOnlineGameLoop(); stopVoice(); stopWaitingPoll(); if (currentOnlineGame) sessionStorage.removeItem('chess3d_playerkey_' + currentOnlineGame.id); currentOnlineGame = null; sessionPlayerKey = null; myColor = 'w'; moveSyncing = false; lastKnownServerState = null; over = false; frozen = false; engine.setFrozen(false); ui.hideGameUI(); ui.hideGameOver(); engine.resetState(); started = false; gameMode = null; }
