// ui_handler.js — Chess 3D (event‑delegation – all buttons work)
...
export function initUI(cb) {
    ...
    // Attach one delegated click listener for everything
    document.body.addEventListener('click', handleDelegatedClick);

    if ($('debug-overlay')) $('debug-overlay').textContent = 'UI loaded';
...
function handleDelegatedClick(e) {
    ...
    // Log clicks for debugging (optional)
    ...
    switch (id) {
        ...
        case 'home-play-online':
            ...
            break;
        case 'card-2p':
            if (callbacks.onStart2P) callbacks.onStart2P();
            break;
        case 'card-ai':
            if (callbacks.onStartAI) callbacks.onStartAI();
            break;
        case 'card-friends':
        case 'card-history':
        case 'card-settings':
        ...
        case 'btn-restore-local':
            if (callbacks.onRestoreLocal) callbacks.onRestoreLocal();
            break;
        case 'btn-sync-offline-cloud':
            if (callbacks.onSyncOfflineCloud) callbacks.onSyncOfflineCloud();
            break;
        case 'btn-restore-offline-cloud':
            if (callbacks.onRestoreOfflineCloud) callbacks.onRestoreOfflineCloud();
            break;
        case 'btn-delete-all-synced':
            const dp = document.getElementById('delete-confirm-panel');
        ...
        case 'undo-btn':
            ...
            break;
        case 'mode-btn':
            if (callbacks.onModeBtn) callbacks.onModeBtn();
            break;
        ...
        case 'mic-toggle-btn':
            ...
            break;
        case 'speaker-toggle-btn':
            if (callbacks.onToggleSpeaker) callbacks.onToggleSpeaker();
            break;

        // Drawer
        case 'drawer-toggle-left':
            toggleLeftDrawer();
            break;
        ...
        // Lobby
        case 'lobby-start-btn':
            if (callbacks.onCountdownFinished) callbacks.onCountdownFinished();
            break;
        case 'lobby-leave-btn':
            if (callbacks.onCancelWaiting) callbacks.onCancelWaiting();
            break;
        case 'lobby-rematch-btn':
            if (callbacks.onAcceptRematch) callbacks.onAcceptRematch();
            break;

        // Legacy panels
        case 'btn-delete-confirm':
            const dp2 = document.getElementById('delete-confirm-panel');
            if (dp2) dp2.classList.remove('show');
            if (callbacks.onDeleteSynced) callbacks.onDeleteSynced();
            break;
        case 'btn-delete-cancel':
            const dp3 = document.getElementById('delete-confirm-panel');
            if (dp3) dp3.classList.remove('show');
            break;
        case 'btn-exit-save':
            if (callbacks.onExitSave) callbacks.onExitSave();
            break;
        case 'btn-exit-no-save':
            if (callbacks.onExitWithoutSave) callbacks.onExitWithoutSave();
            break;
        case 'btn-exit-cancel':
            const ecp = document.getElementById('exit-choice-panel');
            if (ecp) ecp.classList.remove('show');
            break;
        case 'btn-exit-online-yes':
            if (callbacks.onExitOnlineYes) callbacks.onExitOnlineYes();
            break;
        case 'btn-exit-online-stay':
            const eop = document.getElementById('exit-online-panel');
            if (eop) eop.classList.remove('show');
            break;
        case 'btn-restore-ai':
            const rcp = document.getElementById('restore-choice-panel');
            if (rcp) rcp.classList.remove('show');
            if (callbacks.onRestoreAI) callbacks.onRestoreAI();
            break;
        case 'btn-restore-2p':
            const rcp2 = document.getElementById('restore-choice-panel');
            if (rcp2) rcp2.classList.remove('show');
            if (callbacks.onRestore2P) callbacks.onRestore2P();
            break;
        case 'btn-restore-cancel':
            const rcp3 = document.getElementById('restore-choice-panel');
            if (rcp3) rcp3.classList.remove('show');
            break;
        case 'btn-cloud-restore-ai':
            const ccp = document.getElementById('cloud-choice-panel');
            if (ccp) ccp.classList.remove('show');
            if (callbacks.onCloudRestoreAI) callbacks.onCloudRestoreAI();
            break;
        case 'btn-cloud-restore-2p':
            const ccp2 = document.getElementById('cloud-choice-panel');
            if (ccp2) ccp2.classList.remove('show');
            if (callbacks.onCloudRestore2P) callbacks.onCloudRestore2P();
            break;
        case 'btn-cloud-restore-cancel':
            const ccp3 = document.getElementById('cloud-choice-panel');
            if (ccp3) ccp3.classList.remove('show');
            break;

        // Online flow
        case 'btn-public-menu':
            hideAllPanels();
            const pm = document.getElementById('public-menu');
            if (pm) pm.classList.add('show');
            break;
        case 'btn-private-menu':
            hideAllPanels();
            const pvm = document.getElementById('private-menu');
            if (pvm) pvm.classList.add('show');
            break;
        case 'btn-online-back':
            hideAllPanels();
            if (callbacks.showMenu) callbacks.showMenu();
            break;
        case 'btn-create-public':
            if (callbacks.onCreatePublicRoom) callbacks.onCreatePublicRoom();
            break;
        case 'btn-join-public':
            if (callbacks.onJoinPublicRoom) callbacks.onJoinPublicRoom();
            break;
        case 'btn-public-back':
            hideAllPanels();
            const om2 = document.getElementById('online-menu');
            if (om2) om2.classList.add('show');
            break;
        case 'btn-show-create-private':
            if (callbacks.onCreatePrivateRoom) callbacks.onCreatePrivateRoom();
            break;
        case 'btn-show-join-private':
            hideAllPanels();
            const jp = document.getElementById('join-private');
            if (jp) jp.classList.add('show');
            break;
        case 'btn-private-back':
            hideAllPanels();
            const om3 = document.getElementById('online-menu');
            if (om3) om3.classList.add('show');
            break;
        case 'btn-join-private':
            if (callbacks.onJoinPrivateRoom) callbacks.onJoinPrivateRoom();
            break;
        case 'btn-join-private-back':
            hideAllPanels();
            const pvm2 = document.getElementById('private-menu');
            if (pvm2) pvm2.classList.add('show');
            break;
        case 'btn-cancel-waiting':
            if (callbacks.onCancelWaiting) callbacks.onCancelWaiting();
            break;
        case 'btn-rematch-accept':
            if (callbacks.onAcceptRematch) callbacks.onAcceptRematch();
            break;
        case 'btn-rematch-decline':
            if (callbacks.onDeclineRematch) callbacks.onDeclineRematch();
            break;
        case 'btn-go-login':
            window.location.href = 'user_login.html';
            break;
        case 'btn-login-gate-back':
            hideAllPanels();
            if (callbacks.showMenu) callbacks.showMenu();
            break;
        ...
        // AI
        case 'btn-ai-novice':
            engine.setAiDepth(1); hideAllPanels(); showPanel('ai-color-panel');
            break;
        case 'btn-ai-knight':
            engine.setAiDepth(3); hideAllPanels(); showPanel('ai-color-panel');
            break;
        case 'btn-ai-master':
            engine.setAiDepth(5); hideAllPanels(); showPanel('ai-color-panel');
            break;
        case 'btn-ai-diff-back':
            hideAllPanels(); if (callbacks.showMenu) callbacks.showMenu();
            break;
        case 'btn-ai-red':
            engine.setPlayerColor('w'); startAiCountdown();
            break;
        case 'btn-ai-black':
            engine.setPlayerColor('b'); startAiCountdown();
            break;
        case 'btn-ai-color-back':
            hideAllPanels(); showPanel('ai-diff-panel');
            break;
        case 'btn-cancel-ai-countdown':
            cancelAiCountdown();
            break;
        ...
        default:
            // Do nothing for unknown buttons
            break;
    }
}
...
export function showLobbyPanel(opponentName, roomCode) {
    ...
    const name = document.getElementById('lobby-opponent-name');
    if (name) name.textContent = opponentName || 'Waiting…';
    const start = document.getElementById('lobby-start-btn');
    const leave = document.getElementById('lobby-leave-btn');
    const rematch = document.getElementById('lobby-rematch-btn');
...
export function setVoiceControlsVisibility(visible) {
    ...
    if (vc) vc.style.display = visible ? '' : 'none';
}
...
export function showMenu() {
    const ms = document.getElementById('ms'); if (ms) ms.style.display = 'flex';
    const gu = document.getElementById('gu'); if (gu) gu.style.display = 'none';
}
...
export function showPromotion(color) {
    const po = document.getElementById('po'); if (!po) return;
    po.innerHTML = '';
    ['Q','R','B','N'].forEach(t => {
        ...
        btn.addEventListener('click', () => {
            const pm = document.getElementById('pm'); if (pm) pm.classList.remove('on');
...
export function getPrivateRoomCode() {
    const input = document.getElementById('private-room-code');
...
export function showLoginGate() {
    hideAllPanels();
    const gate = document.getElementById('login-gate-panel');
...
export function showExitChoicePanel() {
    const panel = document.getElementById('exit-choice-panel');
...
export function showExitOnlinePanel() {
    const panel = document.getElementById('exit-online-panel');
    if (panel) panel.classList.add('show');
}
